 ...
 * renders the same answer full-width. There must be exactly ONE implementation of the verdict
 * vocabulary: two copies drift, and the copy that drifts is the one that starts rounding
 * `indeterminate` to a yes or a no — which is the entire failure this verdict exists to stop.
 */
...
export const VERDICT: Record<IamVerdict, { label: string; box: string; icon: string }> = {
...
/** The common-action menu plus a free-text field. Committing on Enter or on "Check" — never on
 *  every keystroke — because each commit is a full evaluation on the server. */
export function ActionPicker({ value, onChange }: { value: string; onChange: (action: string) => void }) {
  const [pending, setPending] = useState(value);
  return (
    <>
      <select
        value={COMMON_ACTIONS.some((c) => c.action === value) ? value : ""}
        onChange={(e) => {
          if (e.target.value) {
...
/** The verdict and its full evidence chain. Shared by the row drawer and the Effective Access
 *  tab so the two can never disagree about what an answer looks like. */
export function DecisionResult({ d }: { d: IamDecision }) {
  const v = VERDICT[d.verdict];
  return (
    <>
      <div className={`rounded-lg border p-3 ${v.box}`} data-testid="iam-verdict" data-verdict={d.verdict}>
...
export function WhyPanel({
  ...
  // Deep link into the full tab with this question already asked, so a reader who wants the
  // inverse pivot ("who else can do this?") does not have to retype anything.
  ...
  return (
    <div className="flex h-full min-h-0 w-[28rem] flex-col border-l bg-white">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-auto p-3">
        ...
        {d && <DecisionResult d={d} />}
        {d && (
          <a href={deepLink} className="block pt-1 text-[11px] text-brand hover:underline">
            Open in Effective Access →
          </a>
        )}
      </div>
    </div>
