...
const ACTIVITY_CATEGORIES = ["Administrative", "ServiceHealth", "ResourceHealth", "Security", "Recommendation"] as const;
const SERVICE_HEALTH_INCIDENT_TYPES = ["Incident", "Maintenance", "Informational", "ActionRequired", "Security"] as const;
const RESOURCE_HEALTH_EVENT_STATUSES = ["Active", "InProgress", "Resolved", "Updated"] as const;
const RESOURCE_HEALTH_CURRENT_STATUSES = ["Available", "Degraded", "Unavailable"] as const;
const RESOURCE_HEALTH_PREVIOUS_STATUSES = ["Available", "Degraded", "Unavailable", "Unknown"] as const;
const RESOURCE_HEALTH_REASON_TYPES = ["PlatformInitiated", "Unknown", "UserInitiated"] as const;
const RECOMMENDATION_CATEGORIES = ["Cost", "Performance", "HighAvailability", "OperationalExcellence", "Security"] as const;
const RECOMMENDATION_IMPACTS = ["High", "Medium", "Low"] as const;
const ACTIVITY_LEVELS = ["Critical", "Error", "Warning", "Informational", "Verbose"] as const;
const ACTIVITY_STATUSES = ["Started", "InProgress", "Succeeded", "Failed", "Active", "Resolved", "Updated"] as const;

const ACTIVITY_VALUE_LABELS: Record<string, string> = {
...
function defaultActivityConditions(category: string): Array<Record<string, unknown>> {
  const conditions: Array<Record<string, unknown>> = [{ field: "category", equals: category }];
  if (category === "ServiceHealth") conditions.push({ field: "properties.incidentType", containsAny: [...SERVICE_HEALTH_INCIDENT_TYPES] });
  if (category === "ResourceHealth") conditions.push(
    { field: "status", containsAny: [...RESOURCE_HEALTH_EVENT_STATUSES] },
    { field: "properties.currentHealthStatus", containsAny: [...RESOURCE_HEALTH_CURRENT_STATUSES] },
    { field: "properties.previousHealthStatus", containsAny: [...RESOURCE_HEALTH_PREVIOUS_STATUSES] },
    { field: "properties.cause", containsAny: [...RESOURCE_HEALTH_REASON_TYPES] },
  );
  if (category === "Recommendation") conditions.push(
    { field: "properties.recommendationCategory", containsAny: [...RECOMMENDATION_CATEGORIES] },
    { field: "properties.recommendationImpact", containsAny: [...RECOMMENDATION_IMPACTS] },
...
function canonicalArmId(value: string): string {
...
function groupedActionGroups(actionGroups: ManagedActionGroup[], subscriptionNames: Record<string, string> = {}) {
  ...
  return Array.from(groups.values())
    .map((items) => ({
      ...
      subscriptionName: actionGroupSubscriptionName(items[0]) || subscriptionNames[actionGroupSubscriptionId(items[0]).toLowerCase()] || "",
...
function ActionGroupChoices({ actionGroups, subscriptionNames, selectedIds, ruleSubscriptionId, onToggle }: {
  ...
  const selectedKeys = new Set(selectedIds.map(canonicalArmId));
  const visibleKeys = new Set(actionGroups.map((group) => canonicalArmId(group.id)));
  const unavailableSelected = selectedIds.filter((id) => !visibleKeys.has(canonicalArmId(id)));
  return <div className="mt-2 space-y-3">{groupedActionGroups(actionGroups, subscriptionNames).map((subscription) => <section key={subscription.subscriptionId || "unknown"} className="overflow-hidden rounded-lg border bg-gray-50/60">
    ...
    <div className="grid gap-2 p-2 sm:grid-cols-2">{subscription.items.map((group) => {
      ...
      return <label key={group.id} className={`flex items-start gap-2 rounded border bg-white px-2.5 py-2 ${crossSubscription ? "border-amber-300" : "border-gray-200"}`}>
        <input type="checkbox" className="mt-0.5" checked={selectedKeys.has(canonicalArmId(group.id))} onChange={(event) => onToggle(group, event.target.checked)} />
  ...
  </section>)}{unavailableSelected.length > 0 && <section className="overflow-hidden rounded-lg border border-amber-300 bg-amber-50"><div className="border-b border-amber-200 px-3 py-1.5 text-[11px] font-semibold text-amber-800">Referenced by rule — unavailable or outside visible scope</div><div className="space-y-1 p-2">{unavailableSelected.map((id) => <div key={id} className="break-all rounded border border-amber-200 bg-white px-2.5 py-2 font-mono text-[10px] text-amber-800">{id}</div>)}</div></section>}</div>;
}
...
export function newAlertRule(family: RuleFamily, subscriptionId = ""): EditableAlertRule {
  const base: EditableAlertRule = {
    ...
    activity_conditions: family === "activity" ? defaultActivityConditions("Administrative") : [],
    display_name: "",
...
function ActivityConditionDropdown({ label, options, selected, onChange, required = false }: { label: string; options: readonly string[]; selected: string[]; onChange: (values: string[]) => void; required?: boolean }) {
  const [search, setSearch] = useState("");
  const detailsRef = useRef<HTMLDetailsElement | null>(null);
  const customValues = selected.filter((value) => !options.includes(value));
  const visible = options.filter((option) => !search || (ACTIVITY_VALUE_LABELS[option] || option).toLowerCase().includes(search.toLowerCase()));
  ...
  const knownSelected = selected.filter((value) => options.includes(value)).length;
  const summary = knownSelected === options.length && !customValues.length ? `${knownSelected} selected` : selected.length ? `${selected.length} selected` : "Select values";
  return <div className="min-w-0"><div className="mb-1 text-[11px] font-medium text-gray-700">{label}{required && <span className="ml-1 text-red-600">*</span>}</div><details ref={detailsRef} className="relative"><summary className={`list-none cursor-pointer rounded border bg-white px-3 py-2 text-xs ${required && !selected.length ? "border-red-400" : "border-gray-300"}`}>{summary}<span className="float-right">⌄</span></summary><div className="absolute z-30 mt-1 w-full min-w-60 rounded-lg border bg-white p-2 shadow-xl"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Type to filter…" className="mb-2 w-full rounded border px-2 py-1.5 text-xs" /><div className="mb-1 flex justify-between text-[10px]"><button type="button" onClick={() => onChange([...customValues, ...options])} className="text-blue-700">Select all</button><button type="button" onClick={() => onChange([])} className="text-gray-600">Clear</button></div><div className="max-h-56 space-y-0.5 overflow-auto">{visible.map((option) => <label key={option} className="flex cursor-pointer items-center gap-2 rounded px-1 py-1.5 hover:bg-gray-50"><input type="checkbox" checked={selected.includes(option)} onChange={() => toggle(option)} /><span>{ACTIVITY_VALUE_LABELS[option] || option}</span></label>)}</div>{!visible.length && <div className="py-3 text-center text-[10px] text-gray-400">No matches</div>}</div></details>{customValues.length > 0 && <div className="mt-1 text-[9px] text-amber-700">Custom values preserved: {customValues.join(", ")}</div>}</div>;
}

function ActivityConditionsEditor({ category, valueFor, valuesFor, onCategoryChange, onSetEquals, onSetValues }: { category: string; valueFor: (field: string) => string; valuesFor: (field: string) => string[]; onCategoryChange: (category: string) => void; onSetEquals: (field: string, value: string) => void; onSetValues: (field: string, values: string[]) => void }) {
  const commonFields = category === "Administrative" || category === "Security";
  return <section className="space-y-4 rounded-lg border p-3"><div><h3 className="font-semibold text-gray-800">Activity Log conditions</h3><p className="text-[10px] text-gray-500">Choose a category to load its supported Azure condition fields. Values are written to the ARM rule as equals or containsAny conditions.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><label className="text-[11px] font-medium text-gray-700">Category<select value={category} onChange={(event) => onCategoryChange(event.target.value)} className="mt-1 w-full rounded border border-gray-300 bg-white px-2 py-2 text-xs">{ACTIVITY_CATEGORIES.map((option) => <option key={option} value={option}>{activityCategoryLabel(option)}</option>)}</select></label>
    {category === "ServiceHealth" && <><ActivityConditionDropdown label="Event types" options={SERVICE_HEALTH_INCIDENT_TYPES} selected={valuesFor("properties.incidentType")} onChange={(values) => onSetValues("properties.incidentType", values)} required /><label className="text-[11px] font-medium text-gray-700">Impacted services<input value={valuesFor("properties.impactedServices[*].ServiceName").join(", ")} onChange={(event) => onSetValues("properties.impactedServices[*].ServiceName", event.target.value.split(",").map((item) => item.trim()).filter(Boolean))} placeholder="All services" className="mt-1 w-full rounded border px-2 py-2 text-xs" /></label><label className="text-[11px] font-medium text-gray-700">Impacted regions<input value={valuesFor("properties.impactedServices[*].ImpactedRegions[*].RegionName").join(", ")} onChange={(event) => onSetValues("properties.impactedServices[*].ImpactedRegions[*].RegionName", event.target.value.split(",").map((item) => item.trim()).filter(Boolean))} placeholder="All regions" className="mt-1 w-full rounded border px-2 py-2 text-xs" /></label></>}
    {category === "ResourceHealth" && <><ActivityConditionDropdown label="Event status" options={RESOURCE_HEALTH_EVENT_STATUSES} selected={valuesFor("status")} onChange={(values) => onSetValues("status", values)} required /><ActivityConditionDropdown label="Current resource status" options={RESOURCE_HEALTH_CURRENT_STATUSES} selected={valuesFor("properties.currentHealthStatus")} onChange={(values) => onSetValues("properties.currentHealthStatus", values)} required /><ActivityConditionDropdown label="Previous resource status" options={RESOURCE_HEALTH_PREVIOUS_STATUSES} selected={valuesFor("properties.previousHealthStatus")} onChange={(values) => onSetValues("properties.previousHealthStatus", values)} required /><ActivityConditionDropdown label="Reason type" options={RESOURCE_HEALTH_REASON_TYPES} selected={valuesFor("properties.cause")} onChange={(values) => onSetValues("properties.cause", values)} required /></>}
    {category === "Recommendation" && <><ActivityConditionDropdown label="Recommendation category" options={RECOMMENDATION_CATEGORIES} selected={valuesFor("properties.recommendationCategory")} onChange={(values) => onSetValues("properties.recommendationCategory", values)} required /><ActivityConditionDropdown label="Impact level" options={RECOMMENDATION_IMPACTS} selected={valuesFor("properties.recommendationImpact")} onChange={(values) => onSetValues("properties.recommendationImpact", values)} required /><div className="rounded border border-sky-200 bg-sky-50 p-2 text-[10px] leading-4 text-sky-800"><strong>Advisor signal</strong><div>Operation is fixed to Microsoft.Advisor/recommendations/available/action.</div></div></>}
    {commonFields && <><label className="text-[11px] font-medium text-gray-700">Operation name<input value={valueFor("operationName")} onChange={(event) => onSetEquals("operationName", event.target.value)} placeholder={category === "Security" ? "Microsoft.Security/…" : "Microsoft.Compute/virtualMachines/write"} className="mt-1 w-full rounded border px-2 py-2 text-xs" /></label><label className="text-[11px] font-medium text-gray-700">Level<select value={valueFor("level")} onChange={(event) => onSetEquals("level", event.target.value)} className="mt-1 w-full rounded border px-2 py-2 text-xs"><option value="">All levels</option>{ACTIVITY_LEVELS.map((option) => <option key={option}>{option}</option>)}</select></label><label className="text-[11px] font-medium text-gray-700">Status<select value={valueFor("status")} onChange={(event) => onSetEquals("status", event.target.value)} className="mt-1 w-full rounded border px-2 py-2 text-xs"><option value="">All statuses</option>{ACTIVITY_STATUSES.map((option) => <option key={option} value={option}>{ACTIVITY_VALUE_LABELS[option] || option}</option>)}</select></label><label className="text-[11px] font-medium text-gray-700">Resource type<input value={valueFor("resourceType")} onChange={(event) => onSetEquals("resourceType", event.target.value)} placeholder="Microsoft.Compute/virtualMachines" className="mt-1 w-full rounded border px-2 py-2 text-xs" /></label><label className="text-[11px] font-medium text-gray-700">Resource group<input value={valueFor("resourceGroup")} onChange={(event) => onSetEquals("resourceGroup", event.target.value)} className="mt-1 w-full rounded border px-2 py-2 text-xs" /></label></>}
  </div></section>;
}
...
export function AlertRuleEditor({ initial, connectionId, workloadId = "", actionGroups, canPreview, busy, onClose, onSave }: {
  ...
  useEffect(() => {
    let active = true;
    api.alertsAuthoringOptions({ connection_id: connectionId }).then((options) => {
      if (!active) return;
      setSubscriptionNames(Object.fromEntries(options.subscriptions.map((item) => [item.id.toLowerCase(), item.name])));
    }).catch(() => { /* Action Group IDs remain usable when subscription labels cannot be loaded. */ });
    return () => { active = false; };
  }, [connectionId]);
  ...
  const activityValues = (field: string) => {
    const item = (value.activity_conditions || []).find((condition) => String(condition.field || "").toLowerCase() === field.toLowerCase());
    if (Array.isArray(item?.containsAny)) return (item.containsAny as unknown[]).map(String);
    return item?.equals ? [String(item.equals)] : [];
  };
  ...
  const setActivityValues = (field: string, values: string[]) => setValue((current) => ({ ...current, activity_conditions: [...(current.activity_conditions || []).filter((condition) => String(condition.field || "").toLowerCase() !== field.toLowerCase()), ...(values.length ? [{ field, containsAny: [...new Set(values)] }] : [])] }));
  const changeActivityCategory = (category: string) => setValue((current) => ({ ...current, category, activity_conditions: defaultActivityConditions(category) }));
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={`${initial.id ? "Edit" : "Create"} ${family} alert rule`}>
    <div className="max-h-[94vh] w-full max-w-5xl overflow-auto rounded-xl bg-white shadow-xl">
      ...
      <div className="space-y-5 p-5 text-xs">
        ...
        {family === "activity" && <ActivityConditionsEditor category={activityValue("category") || "Administrative"} valueFor={activityValue} valuesFor={activityValues} onCategoryChange={changeActivityCategory} onSetEquals={setActivity} onSetValues={setActivityValues} />}
        ...
        {family === "prometheus" && <PrometheusRulesEditor value={value} setValue={setValue} actionGroups={actionGroups} subscriptionNames={subscriptionNames} ruleSubscriptionId={value.subscription_id} />}
...
function PrometheusRulesEditor({ value, setValue, actionGroups, subscriptionNames, ruleSubscriptionId }: { value: EditableAlertRule; setValue: Dispatch<SetStateAction<EditableAlertRule>>; actionGroups: ManagedActionGroup[]; subscriptionNames: Record<string, string>; ruleSubscriptionId: string }) {
