...
export function ManagedAlertRulesTable({ rows, caps, busy, actionGroups = [], onCreate, onEdit, onClone, onToggle, onDelete, onBulk }: {
  ...
  return (
    <div className="space-y-3">
      ...
      {caps?.can_bulk_manage && selected.length > 0 && <div className="flex flex-wrap items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-xs"><strong>{selected.length} selected</strong><select value={bulkAction} onChange={(event) => setBulkAction(event.target.value as typeof bulkAction)} className="rounded border px-2 py-1"><option value="enable">Enable</option><option value="disable">Disable</option><option value="add_action_group">Add Action Group</option><option value="delete">Delete</option></select>{bulkAction === "add_action_group" && <select value={bulkGroup} onChange={(event) => setBulkGroup(event.target.value)} className="min-w-56 rounded border px-2 py-1"><option value="">Choose Action Group…</option>{actionGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select>}<button disabled={!!busy || (bulkAction === "add_action_group" && !bulkGroup)} onClick={() => onBulk?.(rows.filter((row) => selected.includes(row.id)), bulkAction, bulkGroup)} className="rounded bg-indigo-700 px-3 py-1 text-white disabled:opacity-50">Create {selected.length} change requests</button><button onClick={() => setSelected([])} className="text-gray-600">Clear</button><span className="ml-auto text-gray-500">Not atomic; each child change is approved, applied, and rolled back independently.</span></div>}
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={`${initial.id ? "Edit" : "Create"} ${family} alert rule`}>
    <div className="max-h-[94vh] w-full max-w-5xl overflow-auto rounded-xl bg-white shadow-xl">
      ...
      <div className="space-y-5 p-5 text-xs">
        ...
        {family === "prometheus" && <PrometheusRulesEditor value={value} setValue={setValue} actionGroups={actionGroups} />}
        ...
        {family !== "prometheus" && <section className="rounded-lg border p-3"><div className="flex items-center"><div><h3 className="font-semibold text-gray-800">Action Groups</h3><p className="text-[10px] text-gray-500">Ownership routing compares resolved owners server-side; receiver destinations are shown in full.</p></div><button disabled={suggesting || (!value.scopes[0] && !workloadId)} onClick={() => void suggestRouting()} className="ml-auto rounded border px-2 py-1 text-indigo-700 disabled:opacity-50">{suggesting ? "Matching…" : "Suggest from ownership"}</button></div>{suggestions.length > 0 && <div className="mt-2 rounded border border-indigo-200 bg-indigo-50 p-2"><div className="mb-1 font-medium text-indigo-800">Suggested routing</div>{suggestions.slice(0, 5).map((item) => <div key={item.action_group_id} className="flex items-center gap-2 py-1"><span>{item.name}</span><span className="text-[10px] text-indigo-600">{Math.round(item.confidence * 100)}% · {item.reason}</span><button disabled={value.action_group_ids.includes(item.action_group_id)} onClick={() => setValue({ ...value, action_group_ids: [...value.action_group_ids, item.action_group_id] })} className="ml-auto rounded border border-indigo-300 px-2 py-0.5 text-[10px] text-indigo-700 disabled:opacity-40">Add</button></div>)}</div>}<div className="mt-2 grid gap-2 sm:grid-cols-2">{actionGroups.map((group) => <label key={group.id} className="flex items-center gap-2 rounded border px-2 py-1.5"><input type="checkbox" checked={value.action_group_ids.includes(group.id)} onChange={(event) => setValue({ ...value, action_group_ids: event.target.checked ? [...value.action_group_ids, group.id] : value.action_group_ids.filter((id) => id !== group.id) })} /><span>{group.name}</span><span className="ml-auto text-[10px] text-gray-400">{group.receiver_count} receivers</span></label>)}</div>{actionGroups.length === 0 && <p className="mt-2 text-gray-400">No Action Groups in this scope. Create one from the Action groups tab first.</p>}</section>}
...
function PrometheusRulesEditor({ value, setValue, actionGroups }: { value: EditableAlertRule; setValue: Dispatch<SetStateAction<EditableAlertRule>>; actionGroups: ManagedActionGroup[] }) {
