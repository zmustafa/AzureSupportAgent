  ...
  const filtered = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      if (sortKey === "worst") return Number(b.has_scan) - Number(a.has_scan)
        ...
        || b.unknown_destinations - a.unknown_destinations
      ...
      const value = (row: TelemetryCoverageFleetRow) => {
        if (!row.has_scan) return -1;
        if (sortKey === "coverage") return row.coverage_pct ?? -1;
        if (sortKey === "resources") return row.resources;
        if (sortKey === "with_diag") return row.with_any_diag;
        if (sortKey === "all_categories") return row.resources ? (row.with_all_categories / row.resources) * 100 : 0;
        return row.unknown_destinations;
      };
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="border-b bg-white px-5 py-3">
      <div className="flex flex-wrap items-center gap-3">
        <div className="min-w-0"><h2 className="text-sm font-semibold text-gray-900">Fleet telemetry coverage</h2><p className="text-[11px] text-gray-500">Latest diagnostic-settings scan per workload. Select workloads and scan them as one background fleet.</p></div>
      ...
      {fleetQ.isLoading ? <Skeleton rows={8} /> : fleetQ.isError ? <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{formatError(fleetQ.error)}</div> : !rows.length ? <div className="rounded-md border border-dashed bg-gray-50 px-4 py-10 text-center text-sm text-gray-500">No workloads exist yet.</div> :
        <table className="w-full text-[12px]"><thead className="sticky top-0 z-10 bg-gray-50 text-left text-gray-500"><tr className="border-b">
          ...
          return <tr key={row.workload_id} className={`border-b hover:bg-gray-50 ${selected.has(row.workload_id) ? "bg-brand/5" : ""}`}>
            ...
            <td className="px-2 py-1.5">{running ? <span className="inline-flex items-center gap-1 text-brand"><span className="animate-spin">↻</span>scanning…</span> : queued ? <span className="text-gray-400">queued</span> : error ? <span className="rounded bg-red-50 px-1.5 py-0.5 font-medium text-red-700" title={error}>⚠ failed</span> : !row.has_scan ? <span className="text-gray-400">never</span> : <PercentPill value={row.coverage_pct} />}</td>
            <td className="px-2 py-1.5 tabular-nums text-gray-600">{row.has_scan ? row.resources : "—"}</td><td className="px-2 py-1.5 tabular-nums text-gray-600">{row.has_scan ? row.with_any_diag : "—"}</td><td className="px-2 py-1.5 tabular-nums text-gray-600">{row.has_scan ? `${row.resources ? Math.round((row.with_all_categories / row.resources) * 100) : 0}%` : "—"}</td><td className={`px-2 py-1.5 tabular-nums ${row.unknown_destinations ? "text-amber-600" : "text-gray-400"}`}>{row.has_scan ? row.unknown_destinations : "—"}</td><td className="px-2 py-1.5 text-gray-500" title={error || row.run_at}>{running ? "live Azure scan" : error ? "failed — retry" : relTime(row.run_at)}</td><td className="px-2 py-1.5"><button onClick={() => onOpenWorkload(row.workload_id, row.connection_id)} className="rounded border px-2 py-0.5 text-[11px] text-gray-600 hover:bg-gray-50">Open ▸</button></td>
          </tr>;
        })}</tbody></table>}
    </div>
  </div>;
