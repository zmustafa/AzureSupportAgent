...
export function BackupDrCoverageFleet({ onOpenWorkload }: { onOpenWorkload: (workloadId: string, connectionId: string) => void }) {
  const durable = useDurableBatch("coverage_backupdr", [["backupdrFleet"], ["backupdr-trend"], ["coverage-runs", "backupdr"]]);
  ...
  function launch() {
    ...
    void durable.launch(chosen.map((row) => row.workload_id))
      .then(() => setMessage(`Queued Backup & DR coverage scans for ${chosen.length} workload${chosen.length === 1 ? "" : "s"}. The server owns the complete batch.`))
  ...
  function retryFailed() {
    setBusy("retry");
    void durable.retry().then(() => setMessage(`Queued ${failedRows.length} failed/partial Backup & DR scan${failedRows.length === 1 ? "" : "s"} for retry.`))
      .catch((error) => setMessage(formatError(error))).finally(() => setBusy(""));
  }
