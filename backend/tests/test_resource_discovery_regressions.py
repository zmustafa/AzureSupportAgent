"""Regression tests for resource discovery, connection lookup, and payload parsing.
...
def test_resolve_connection_still_falls_back_by_design():
    """The fallback is intentional, so path-param endpoints that identify a specific
    connection must not use it."""
    import inspect
