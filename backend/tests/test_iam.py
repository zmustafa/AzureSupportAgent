"""Unit tests for the IAM (access review) feature: schema, cache, demo, compose, pivots,
...
from app.iam import cache, compose, demo, export, pivots, schema, scopes
from app.iam.agent_tool import build_iam_tools
from app.iam.store import _privileged_keys
...
def isolated_cache(tmp_path, monkeypatch):
    ...
    # The legacy-path migration is a one-shot per process; disarm it so a tmp cache never
    # reaches for (or moves) anything outside the tmp dir.
...
def test_make_row_has_all_columns():
    ...
    # A row must default to Allow: a collector that forgets to set `effect` would otherwise
    # emit a Deny and silently hide the access it was reporting.
...
def test_scanner_columns_are_frozen():
    """``SCANNER_COLUMNS`` is the import/export contract with the standalone scanner.

    Adding, removing or reordering anything here breaks round-tripping with every workbook and
    CSV the scanner has ever produced. New columns belong on ``EXTRA_COLUMNS``, which is why
    ``COLUMNS`` must start with the scanner set verbatim."""
    assert len(schema.SCANNER_COLUMNS) == 46
    assert schema.SCANNER_COLUMNS[0] == "surface"
    assert schema.SCANNER_COLUMNS[-1] == "errorMessage"
    assert schema.COLUMNS[: len(schema.SCANNER_COLUMNS)] == schema.SCANNER_COLUMNS
    assert schema.COLUMNS == (*schema.SCANNER_COLUMNS, *schema.EXTRA_COLUMNS)
    # No duplicates across the two sets.
    assert len(set(schema.COLUMNS)) == len(schema.COLUMNS)
...
# ------------------------------------------------- legacy cache migration (/rbac -> /iam)
def test_legacy_cache_paths_are_migrated_in_place(tmp_path, monkeypatch):
    """The pre-rename ``.data/rbac*`` cache must move, not be abandoned.

    Without this, every tenant reports "never collected" after the upgrade and a full
    re-collect on a large estate is indistinguishable from data loss."""
    legacy_index = tmp_path / "rbac_cache.json"
    legacy_blobs = tmp_path / "rbac"
    legacy_index.write_text('{"t1": {"scopes": {}}}', encoding="utf-8")
    (legacy_blobs / "t1").mkdir(parents=True)
    (legacy_blobs / "t1" / "abc.json.gz").write_bytes(b"payload")

    monkeypatch.setattr(cache, "_DATA", tmp_path)
    monkeypatch.setattr(cache, "_INDEX", tmp_path / "iam_cache.json")
    monkeypatch.setattr(cache, "_BLOBS", tmp_path / "iam")
    monkeypatch.setattr(cache, "_migrated", False)

    cache._migrate_legacy_paths()

    assert (tmp_path / "iam_cache.json").read_text(encoding="utf-8") == '{"t1": {"scopes": {}}}'
    assert (tmp_path / "iam" / "t1" / "abc.json.gz").read_bytes() == b"payload"
    assert not legacy_index.exists() and not legacy_blobs.exists()

    # Idempotent: a second pass with nothing to move is a no-op, not an error.
    monkeypatch.setattr(cache, "_migrated", False)
    cache._migrate_legacy_paths()
    assert (tmp_path / "iam_cache.json").exists()


def test_legacy_cache_migration_never_clobbers_an_existing_cache(tmp_path, monkeypatch):
    """A half-merged cache is worse than a stale one — leave the legacy copy as a fallback."""
    (tmp_path / "rbac_cache.json").write_text('{"old": 1}', encoding="utf-8")
    (tmp_path / "iam_cache.json").write_text('{"new": 1}', encoding="utf-8")
    ...
    assert (tmp_path / "iam_cache.json").read_text(encoding="utf-8") == '{"new": 1}'
    assert (tmp_path / "rbac_cache.json").exists()  # kept as a manual fallback


def test_legacy_cache_migration_is_scoped_to_the_configured_cache_dir(tmp_path, monkeypatch):
    """Legacy names are derived from the CURRENT paths, never hard-coded at ``.data``.

    Tests monkeypatch the cache dir; a hard-coded legacy path would reach out of the tmp dir
    and move the operator's real cache into it."""
    monkeypatch.setattr(cache, "_DATA", tmp_path)
    monkeypatch.setattr(cache, "_INDEX", tmp_path / "iam_cache.json")
    monkeypatch.setattr(cache, "_BLOBS", tmp_path / "iam")
    monkeypatch.setattr(cache, "_migrated", False)
    ...
    # Nothing existed to migrate, and nothing was created outside the tmp dir.
    assert list(tmp_path.iterdir()) == []


# ------------------------------------------------ renamed settings keys (/rbac -> /iam)
def _settings_from(tmp_path, monkeypatch, saved: dict):
    from app.core import app_settings

    path = tmp_path / "app_settings.json"
    path.write_text(json.dumps(saved), encoding="utf-8")
    monkeypatch.setattr(app_settings, "_PATH", path)
    return app_settings.load_settings()


def test_configured_ttl_survives_the_settings_key_rename(tmp_path, monkeypatch):
    """``load_settings`` filters the stored blob to keys present in DEFAULTS, so a renamed key
    is dropped on read and an operator's configured value silently reverts to the default."""
    s = _settings_from(tmp_path, monkeypatch, {
        "rbac_cache_ttl_s": 900, "rbac_max_rows": 12345, "rbac_tools_enabled": False,
    })
    assert s["iam_cache_ttl_s"] == 900
    assert s["iam_max_rows"] == 12345
    assert s["iam_tools_enabled"] is False


def test_an_explicit_new_key_wins_over_the_legacy_one(tmp_path, monkeypatch):
    s = _settings_from(tmp_path, monkeypatch, {"rbac_cache_ttl_s": 900, "iam_cache_ttl_s": 60})
    assert s["iam_cache_ttl_s"] == 60


def test_settings_defaults_apply_when_neither_key_is_stored(tmp_path, monkeypatch):
    s = _settings_from(tmp_path, monkeypatch, {})
    assert s["iam_cache_ttl_s"] == 21600
    assert s["iam_max_rows"] == 5000
    assert s["iam_tools_enabled"] is True
...
def test_export_csv_and_json(isolated_cache):
    ...
    assert len(header.split(",")) == len(schema.COLUMNS)
    parsed = json.loads(export.to_json(master))
    assert len(parsed) == len(master) and set(parsed[0].keys()) == set(schema.COLUMNS)


def test_scanner_format_export_projects_back_to_the_frozen_46(isolated_cache):
    """`fmt=scanner` must emit exactly the scanner's columns, in order — that is what makes a
    round trip through the import endpoint byte-identical."""
    demo.seed_demo("t1")
    master = compose.build_master_rows("t1")

    csv_text = export.to_csv(master, columns=schema.SCANNER_COLUMNS)
    header = csv_text.splitlines()[0].split(",")
    assert tuple(header) == schema.SCANNER_COLUMNS
    assert "effect" not in header and "imported" not in header

    parsed = json.loads(export.to_json(master, columns=schema.SCANNER_COLUMNS))
...
def _assignment(scope: str, *, aid: str, inherited: bool, sub: str = "") -> dict:
    return schema.make_row(
        surface=schema.SURFACE_AZURE_RBAC, roleName="Owner", roleIsPrivileged=True,
        principalId="u1", effectivePrincipalId="u1", accessPath=schema.PATH_DIRECT,
        assignmentId=aid, scope=scope, isInherited=inherited, subscriptionName=sub,
    )


def test_inherited_copies_of_one_assignment_collapse_to_the_authoritative_row():
    """ARM returns an MG-scoped assignment from EVERY child subscription's query.

    Before dedupe, one grant made at a management group covering 26 subscriptions was reported
    26 times, each attributed to a different subscription — inflating every count and making the
    scope-tree totals meaningless."""
    mg = "/providers/Microsoft.Management/managementGroups/corp"
    aid = f"{mg}/providers/Microsoft.Authorization/roleAssignments/ra-1"
    rows = [
        _assignment(mg, aid=aid, inherited=True, sub="Sub A"),   # inherited copy seen from sub A
        _assignment(mg, aid=aid, inherited=False),               # the authoritative MG copy
        _assignment(mg, aid=aid, inherited=True, sub="Sub B"),   # inherited copy seen from sub B
    ]
    out = compose.dedupe_assignments(rows)
    assert len(out) == 1
    # The winner is the copy collected AT the assignment's own scope, so the grant is attributed
    # to the management group rather than to whichever subscription answered first.
    assert out[0]["isInherited"] is False
    assert out[0]["subscriptionName"] == ""


def test_dedupe_keeps_genuinely_distinct_rows():
    mg = "/providers/Microsoft.Management/managementGroups/corp"
    rows = [
        _assignment(mg, aid="ra-1", inherited=False),
        _assignment(mg, aid="ra-2", inherited=False),           # different assignment
        # Same assignment id, DIFFERENT effective principal: a group expansion. These are
        # distinct people's access and must never collapse into one row.
        {**_assignment(mg, aid="ra-1", inherited=False), "effectivePrincipalId": "u2", "accessPath": schema.PATH_GROUP},
    ]
    assert len(compose.dedupe_assignments(rows)) == 3


def test_dedupe_passes_through_rows_without_an_assignment_id():
    """Ownership and classic-admin rows share no id; keying on a blank would merge them all."""
    rows = [
        schema.make_row(roleName="Service Principal Owner", accessPath=schema.PATH_OWNER, effectivePrincipalId="a"),
        schema.make_row(roleName="Service Principal Owner", accessPath=schema.PATH_OWNER, effectivePrincipalId="b"),
    ]
    assert len(compose.dedupe_assignments(rows)) == 2


def test_dedupe_is_stable_and_idempotent():
    mg = "/providers/Microsoft.Management/managementGroups/corp"
    rows = [_assignment(mg, aid=f"ra-{i}", inherited=False) for i in range(5)]
    once = compose.dedupe_assignments(rows)
    assert [r["assignmentId"] for r in once] == [r["assignmentId"] for r in rows]  # order preserved
    assert compose.dedupe_assignments(once) == once


# ---------------------------------------------------------------- deny assignments
def test_deny_assignments_are_not_counted_as_grants(isolated_cache):
    """A deny REMOVES access. Folding it into "Total grants" would inflate the headline number
    with rows that mean the opposite of a grant."""
    demo.seed_demo("t1")
    master = compose.build_master_rows("t1")
    denies = [r for r in master if r["effect"] == schema.EFFECT_DENY]
    assert len(denies) == 1
    assert denies[0]["surface"] == schema.SURFACE_DENY
    # A deny is never flagged privileged — it grants nothing.
    assert denies[0]["roleIsPrivileged"] is False

    ov = compose.compute_overview("t1")
    assert ov["kpis"]["deny_assignments"] == 1
    assert ov["kpis"]["total_assignments"] == len(master) - 1
    # And it must not leak into the privileged / data-plane counts either.
    assert ov["kpis"]["privileged"] == len([r for r in master if r["roleIsPrivileged"] and r["effect"] != schema.EFFECT_DENY])


def test_overview_counts_the_non_rbac_surfaces(isolated_cache):
    demo.seed_demo("t1")
    k = compose.compute_overview("t1")["kpis"]
    assert k["key_vault_policies"] == 1
    assert k["classic_admins"] == 1
...
def test_a_principal_the_directory_could_not_name_is_not_declared_to_exist():
    ...
    `pid in index`, which is true for every row by construction — all 5,506 rows came back
    ``true`` and not one orphan could ever be reported. Graph confirmed 26 of those principals
    return 404: 100 live assignments, including Storage Blob Data Contributor and Key Vault
    access policies, held by identities that no longer exist. Object ids can be recycled against
    a new application, so these are exactly the rows that matter."""
    directory = {"principals": [{"principalId": "u-1", "displayName": "Alice", "principalType": "User"}],
                 "groups": {}, "rows": []}
    scope_rows = [
        schema.make_row(principalId="u-1", effectivePrincipalId="u-1", roleName="Reader"),
        # The deleted one. Its only trace anywhere is this assignment.
        schema.make_row(principalId="ghost", effectivePrincipalId="ghost", roleName="Contributor"),
    ]
    idx = compose._principal_index(directory, scope_rows)
    assert "ghost" in idx, "the index keys every GUID it sees — that is why membership proves nothing"
    assert not idx["ghost"]["name"]

    rows = compose._apply_principal_existence(list(scope_rows), idx, directory_readable=True)
    assert rows[0]["principalExists"] == schema.EXISTS_TRUE
    assert rows[1]["principalExists"] == schema.EXISTS_FALSE, "a nameless principal is not a live one"


def test_an_unreadable_directory_never_manufactures_an_orphan():
    """The other half: a permissions gap must not be reported as a deleted identity."""
    scope_rows = [schema.make_row(principalId="ghost", effectivePrincipalId="ghost", roleName="Owner")]
    idx = compose._principal_index({"principals": [], "groups": {}, "rows": []}, scope_rows)
    rows = compose._apply_principal_existence(list(scope_rows), idx, directory_readable=False)
    assert rows[0]["principalExists"] == schema.EXISTS_UNKNOWN
...
def test_normalize_principal_type():
    from app.iam.collectors import _normalize_principal_type
...
async def test_agent_tools_read_cache(isolated_cache):
    demo.seed_demo("t1")
    tools = {t.name: t for t in build_iam_tools("t1")}
    # A superset assertion, not equality: pinning the exact set turns every new tool into a
    # failing test that says nothing about correctness. What matters is that these four keep
    # existing under these names — they are LLM-visible and referenced from agent prompts.
    assert {
        "who_can_access",
        "privileged_access_review",
        "effective_access_for_principal",
        "can_principal_do",
    } <= set(tools)
