"""Persistent server-side cache for Ownership coverage snapshots.

Mirrors the coverage-detector cache pattern (amba/telemetry/backupdr): snapshots live on
the Azure Files volume (``backend/.data/ownership_coverage_cache.json``) so they survive
...
Read paths NEVER scan Azure — a cache miss returns ``None`` and the API serves a
``never_loaded`` snapshot until the user presses Refresh (read-cache-only-on-visit)."""
