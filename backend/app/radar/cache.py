"""Persistent server-side cache for Radar snapshots.

Aggregating Service Health + Advisor (+ optional Azure Updates feed) and resolving owners
is slow, so snapshots are cached on the Azure Files volume
(``backend/.data/radar_cache.json``), keyed by ``(tenant, scope_kind, scope_id)``, with a
per-key lock to coalesce concurrent recomputes. Mirrors the coverage-detector caches."""
...
_PATH = Path(__file__).resolve().parents[2] / ".data" / "radar_cache.json"
