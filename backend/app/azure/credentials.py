...
def build_mcp_env(conn: dict[str, Any]) -> tuple[dict[str, str], list[str]]:
    ...
    if method == "service_principal":
        ...
        # Host `az login` session pinned to this tenant. The Azure CLI maintains and
        # auto-refreshes the session, so this is effectively always-on (no pasting).
        env["AZURE_TOKEN_CREDENTIALS"] = "AzureCliCredential"
    else:  # default_chain
        # Use the host DefaultAzureCredential (az login / managed identity).
        env["AZURE_TOKEN_CREDENTIALS"] = "AzureCliCredential"
    return env, cleanup
...
async def get_arm_token(conn: dict[str, Any]) -> tuple[str | None, str | None]:
    ...
    if method == "az_cli_token":
        ...
        return token, None
    # azure_cli and default_chain both use the host Azure CLI session (auto-refreshed).
