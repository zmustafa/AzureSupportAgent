"""Telemetry change-request registry — the "Send to Approval Inbox" path.

Standalone, audited registry of proposed diagnostic-settings remediations (the generated
Bicep / Policy artifact for a set of gaps) awaiting human review. Separate from the agent's
Approval/ToolCall model. The app NEVER auto-applies an approved request — approval records
the human sign-off; the artifact is exported to the customer's own pipeline. Persisted at
