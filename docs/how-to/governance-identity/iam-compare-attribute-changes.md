...
5. Work the coloured rows. Colour encodes direction — widening in red, orange or amber; narrowing in green; a path change in blue, because the access is the same and only the route to it moved.
...
2. Open `/iam/simulator` and add the reversing change to the basket. Add every related change together so their interactions are modelled.
...
**Expected result:** A modelled reversal that distinguishes access genuinely removed from access that only appears to be.
...
| An unfamiliar custom role is reported as `escalated` | Escalation is decided by coarse privilege tier, and an unrecognised custom role is placed at write tier so it is never mislabelled a de-escalation from Reader. Confirm what the role authorizes. |
