...
nav_order: 11
description: Compare standing and eligible privilege, review per-role PIM configuration, analyse activation sessions and what was done during them, and produce an evidence pack.
...
- Entra ID P1 for PIM schedules and Entra ID P2 for PIM depth. A licence gap is reported as unlicensed, not as a missing permission.
...
3. Prioritise tier-0 roles. A weak activation policy on a highly privileged role is worth more than a missing notification on a minor one.
...
1. Open the **Activations** sub-tab. The tiles summarise the window: total sessions, Entra ID versus Azure, tier-0 activations, activations outside the working day, and activations with no reason given.
...
3. Prioritise principals holding power in both planes. A principal who can grant themselves directory roles and also controls subscriptions is a single point of total compromise, and this correlation does not exist in any Microsoft surface.
...
| Out-of-hours counts look wrong | The judgement uses your browser's timezone offset, which is shown beside the tile. Confirm it matches the tenant's working day. |
| A justification reads as unknown rather than missing | The source for that session cannot carry a justification, so no judgement is made. |
